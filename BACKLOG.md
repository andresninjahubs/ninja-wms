...
- **Fix producción: historial de órdenes no se persistía en Prisma (v81)** — `PrismaOrderRepository.save` no escribía la columna `events` (ni `shipTo` en update), así que en producción toda orden volvía sin historial y la factura contaba 0 despachos. Ahora se persiste; y facturación usa `shipment.shippedAt` como respaldo para órdenes despachadas guardadas antes del fix.
